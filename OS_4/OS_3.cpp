/*
3. Напишите программу, осуществляющую дешифрование текста, полученного в результате работы программы № 2.
*/
...
char decrypt_char(char c) {
	...
	return c;  // не буквы не дешифруем
}
	...
	while (true) {
		...
		for (int i = 0; i < buffer_size; i++) {
			...
			if (!buffer.empty[pos]) {
				...
				buffer.data[pos] = decrypt_char(original);
...
DWORD WINAPI writer(LPVOID param)
{
	...
	while (true)
	{
		...
		WaitForSingleObject(hSemEncrypted, 100);
